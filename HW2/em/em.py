...
class EM:
    ...
    def _init_parameters(self, X):
        ...
        self.P = np.array([(labels == k).sum() / N for k in range(self.K)])
    ...
    def _e_step(self, x):
        ...
        gaussians = np.array(
            [
                isotropic_gaussian(x, self.mus[k], self.Ds[k]) * self.P[k]
    ...
    def _m_step(self, x):
        ...
        for k in range(self.K):
            ...
            # update D_k
            self.Ds[k] = (
                np.sum(self.r[k, :, None] * (x - self.mus[k]) ** 2, axis=0) / Ns[k]
            )
    ...
    def fit(self, X):
        ...
        for _ in range(self.max_steps):
            ...
            if (
                len(self.nlls) > 1
                and np.abs(self.nlls[-1] - self.nlls[-2]) < self.epsilon
            ):
                # Compute labels
...
if __name__ == "__main__":
    ...
    # Plot the nll
    plt.figure()
    plt.plot(model.nlls, ls="-", lw=1.5, marker="o", label="log likelihood")
